...
function redeemKey(ss, uuid, keyInput) {
  ...
  // Si la clave tiene fecha fija de vencimiento (Soporte DD-MM-YYYY)
  if (keyData.fixedExpire) {
    let fixedDate = keyData.fixedExpire;
    
    // Si viene como texto '31-12-2026'
    if (typeof fixedDate === 'string' && fixedDate.includes('-')) {
       const parts = fixedDate.split('-');
       if(parts.length === 3) {
          // new Date(Year, MonthIndex, Day)
          fixedDate = new Date(parts[2], parts[1] - 1, parts[0]);
       }
    }
    // Si viene como objeto Date de Google Sheets, ya funciona directo en el constructor
    
    // Validar que sea una fecha futura
    if (!isNaN(new Date(fixedDate).getTime()) && new Date(fixedDate).getTime() > now.getTime()) {
      newExpire = new Date(fixedDate);
    } else {
       // Si la fecha fija ya pasó o es inválida, fallback a 3 días? 
       // Mejor lanzar error si la fecha fija está vencida
       if (keyData.fixedExpire) {
          // Si tenía fecha y no es futura, es que expiró
          // Opcional: throw new Error('Esta clave ha expirado');
          // Por defecto del script anterior: fallback 3 días si no se cumple condición.
          // Ajustamos para respetar el input:
          newExpire.setDate(now.getDate() + 3);
       }
    }
  } else {
    // Si no tiene fecha, default 3 días
    newExpire.setDate(now.getDate() + 3);
  }
  ...
  sendTelegram(`💎 Activacion Exitosa!\nUser: ${uuid}\nPlan: ${keyData.plan}\nVence: ${formatDate(newExpire)}`);
...
function formatUser(uuid, plan, dateObj, pub, ext) {
  ...
  // Límites hardcoded por Plan
  let limits = { publish: 500, extract: 500 }; 
  if (plan === 'PRO' || plan === 'TRIAL') limits = { publish: 999999, extract: 999999 };
  
  // SYSTEM CONFIG (Protección Anti-Hack)
  // Si expira, NO enviamos los selectores críticos
  let config = null;
  if (!isExpired) {
       config = {
           selectors: {
               // Facebook Marketplace Selectors (Obfuscated on Server)
               title: 'label[aria-label="Título"]',
               price: 'label[aria-label="Precio"]',
               desc: 'label[aria-label="Descripción"]',
               location: 'label[aria-label="Ubicación"]'
           }
       };
  }
  ...
  return {
    ...
    limits,
    config: config // Will be null if expired
  };
}
